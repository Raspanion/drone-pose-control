...
//             status = input_vstream.write(MemoryView(frames[frames.size() - 1].data, input_vstream.get_frame_size())); // Writing height * width, 3 channels of uint8
//             if (HAILO_SUCCESS != status)
//                 return status;
...
//     auto pp_thread(std::async(post_processing_all<uint8_t>, std::ref(features), frame_count, std::ref(postprocess_time), std::ref(frames), org_height, org_width, nms_on_hailo, model_type));
